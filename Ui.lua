...
library.isMobile    = (inputservice.TouchEnabled and not inputservice.KeyboardEnabled) or (inputservice:GetPlatform() == Enum.Platform.Android or inputservice:GetPlatform() == Enum.Platform.IOS);
            ...
            if library.open then
                ...
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    if hoverObj and hoverObjData then
                        ...
                        hoverObjData.MouseButton1Down:Fire(Vector2.new(pos.X, pos.Y))
                    end

                    -- // Update Sliders Click (only if not dragging window)
                    if library.draggingSlider ~= nil and not isDraggingWindow then
                        local pos = input.UserInputType == Enum.UserInputType.Touch and input.Position or inputservice:GetMouseLocation()
                        local rel = Vector2.new(pos.X, pos.Y) - library.draggingSlider.objects.background.Object.Position;
    ...
    utility:Connection(inputservice.InputEnded, function(input, gpe)
        if self.hasInit and library.open then
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                if hoverObj and hoverObjData then
                    local pos = input.UserInputType == Enum.UserInputType.Touch and input.Position or inputservice:GetMouseLocation()
                    hoverObjData.MouseButton1Up:Fire(Vector2.new(pos.X, pos.Y))
                end
            elseif input.UserInputType == Enum.UserInputType.MouseButton2 then
                if hoverObj and hoverObjData then
    ...
    utility:Connection(inputservice.InputChanged, function(input, gpe)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            if library.open then
                ...
                mousemove:Fire(pos2d);

                if library.CurrentTooltip ~= nil then
                    tooltipObjects.background.Position = UDim2.new(0,pos2d.X + 15,0,pos2d.Y + 15)
                ...
                for _,v in next, library.drawings do
                    ...
                    if hover and not v.Hover then
                        v.Hover = true;
                        v.MouseEnter:Fire(pos2d);
                    elseif not hover and v.Hover then
                        v.Hover = false;
                        v.MouseLeave:Fire(pos2d);
                    end
                end

                if mb1down then

                    -- // Update Sliders Drag (only if not dragging window)
                    if library.draggingSlider ~= nil and not isDraggingWindow then
                        local rel = pos2d - library.draggingSlider.objects.background.Object.Position;
                        local val = utility:ConvertNumberRange(rel.X, 0 , library.draggingSlider.objects.background.Object.Size.X, library.draggingSlider.min, library.draggingSlider.max);
                        library.draggingSlider:SetValue(val)
                    end
    ...
    function self.NewWindow(data)
        ...
        do
            ...
            if library.isMobile then
                objs.mobileToggle = utility:Draw('Square', {
                    Size = newUDim2(0, 40, 0, 40);
                    Position = newUDim2(0, 10, 0.5, -20);
                    ThemeColor = 'Accent';
                    ZIndex = library.zindexOrder.window + 100;
                    Visible = true;
                })

                objs.mobileToggleBorder = utility:Draw('Square', {
                    Size = newUDim2(1, 2, 1, 2);
                    Position = newUDim2(0, -1, 0, -1);
                    ThemeColor = 'Border';
                    ZIndex = library.zindexOrder.window + 99;
